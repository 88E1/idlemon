...
def is_replacement_request(battle) -> bool:
    """
    True when Showdown is only asking for faint replacements.
    Those requests have forceSwitch set and no `active` move list; the
    non-switching slot must send `pass` (not a move).
    """
    force = getattr(battle, "force_switch", (False, False))
    if not any(force):
        return False
    req = getattr(battle, "request_json", None) or {}
    return not req.get(constants.ACTIVE)
...
def get_pokemon_info_from_condition(condition_string: str):
    ...
    if " " in second_part:
        maxhp_str, status = second_part.split(" ", 1)
    else:
        maxhp_str = ""
        status = None
        for char in second_part:
            if char.isdigit():
                ...
                status = second_part[len(maxhp_str):]
                break
