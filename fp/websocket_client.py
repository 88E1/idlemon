...
class PSWebsocketClient:
    ...
    async def create(cls, username, password, address):
        ...
        # Keepalive tuning: the bot does heavy synchronous work (Smogon spread
        # inference / damage calcs) on the event-loop thread between turns. With
        # the websockets default ping_timeout (20s) a long block makes the client
        # tear down the connection, after which recv() hangs forever (the bot has
        # no reconnect). Disable client-side ping timeouts so loop-blocking can't
        # spuriously kill the connection, and lift the message size cap.
